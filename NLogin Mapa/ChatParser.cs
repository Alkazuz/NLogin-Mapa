...
namespace AdvancedBot.client
{
    ...
    static class ChatParser
    {
        ...
        static ChatParser()
        {
            ...
            string file = $"lang\\{Program.Config.GetStringOrDefault("minecraft-language", "en-US")}.lang";
            if (!File.Exists(file))
            {
                try
                {
                    JObject assets = JObject.Parse(DownloadString("https://s3.amazonaws.com/Minecraft.Download/indexes/1.12.json"));
                    string hash = assets["objects"][$"minecraft/lang/{Program.Config.GetStringOrDefault("minecraft-language", "en-US")}.lang"]["hash"].AsStr();
